...
library.theme = {
    ...
    Surface = Color3.fromRGB(27, 28, 33),
    Surface2 = Color3.fromRGB(36, 37, 43),
    Outline = Color3.fromRGB(66, 68, 77),
    Text = Color3.fromRGB(243, 241, 248),
    Muted = Color3.fromRGB(240, 241, 244),
...
local function create(class, properties)
    ...
    if class == "TextLabel" or class == "TextButton" or class == "TextBox" then
        object.TextColor3 = Color3.fromRGB(240, 241, 244)
...
local function corner(parent, radius)
    local object = create("UICorner", {CornerRadius = UDim.new(0, radius or 4), Parent = parent})
    table.insert(corners, object)
    return object
end
...
function library:Notify(info)
    ...
    corner(holder, 5)
    local notifystroke = stroke(holder, library.theme.GradientEnd)
    gradient(notifystroke)
    ...
    local body = text(holder, info.Text or "Notification", 9, library.theme.Text)
    ...
    gradient(bar)
    tween(holder, 0.3, {GroupTransparency = 0})
    ...
    task.delay(duration, function()
        ...
        tween(holder, 0.25, {GroupTransparency = 1, Position = UDim2.fromOffset(25, 0)})
        tween(notifystroke, 0.12, {Transparency = 1})
...
function library:CreateLoading(info)
    ...
    local card = create("CanvasGroup", {AnchorPoint = Vector2.new(0.5, 1), BackgroundColor3 = Color3.fromRGB(18, 19, 23), GroupTransparency = 1, Position = UDim2.new(0.5, 0, 1, -24), Size = UDim2.fromOffset(320, 88), ZIndex = 91, Parent = overlay})
    corner(card, 4)
    stroke(card, library.theme.GradientEnd)
    local cardscale = create("UIScale", {Scale = 0.86, Parent = card})
    local icon = create("ImageLabel", {BackgroundTransparency = 1, Image = imageid(info.Icon or library.icons.Logo), Position = UDim2.fromOffset(14, 15), Size = UDim2.fromOffset(34, 34), ZIndex = 92, Parent = card})
    corner(icon, 5)
    local title = text(card, info.Title or "Loading slimekrew", 11)
    title.Position = UDim2.fromOffset(58, 15)
    title.Size = UDim2.new(1, -64, 0, 15)
    title.ZIndex = 92
    local status = text(card, info.Text or "Preparing interface...", 9, Color3.fromRGB(129, 114, 114))
    status.Position = UDim2.fromOffset(58, 33)
    status.Size = UDim2.new(1, -64, 0, 13)
    status.ZIndex = 92
    local track = create("Frame", {BackgroundColor3 = Color3.fromRGB(42, 43, 49), BorderSizePixel = 0, Position = UDim2.new(0, 14, 1, -17), Size = UDim2.new(1, -28, 0, 4), ZIndex = 92, Parent = card})
    ...
    function object:Close()
        ...
        task.delay(0.35, function()
            ...
            tween(cardscale, 0.3, {Scale = 0.86})
...
function library:CreateWatermark(info)
    ensuregui()
    if self.watermark and self.watermark.Parent then return self.watermark end
    info = info or {}
    local holder = create("Frame", {AnchorPoint = Vector2.new(1, 0), BackgroundColor3 = Color3.fromRGB(18, 19, 23), Position = UDim2.new(1, -16, 0, 16), Size = UDim2.fromOffset(210, 38), Parent = screen})
    corner(holder, 6)
    stroke(holder, library.theme.GradientEnd)
    local avatar = create("ImageLabel", {BackgroundTransparency = 1, Image = imageid(121154699210338), Position = UDim2.fromOffset(6, 5), Size = UDim2.fromOffset(28, 28), Parent = holder})
    corner(avatar, 14)
    task.spawn(function()
    ...
    local label = text(holder, info.Text or "slimekrew", 10)
    label.Position = UDim2.fromOffset(42, 0)
    label.Size = UDim2.new(1, -48, 1, 0)
    local line = create("Frame", {BackgroundColor3 = library.theme.GradientStart, BorderSizePixel = 0, Position = UDim2.new(0, 0, 1, -2), Size = UDim2.new(1, 0, 0, 2), Parent = holder})
    gradient(line)
    local smoothed = 60
    connect(runservice.RenderStepped, function(delta)
        if not holder.Parent then return end
        smoothed = smoothed * 0.9 + (1 / math.max(delta, 1 / 1000)) * 0.1
        local ping = 0
        pcall(function() ping = math.floor(stats.Network.ServerStatsItem["Data Ping"]:GetValue()) end)
        label.Text = string.format("%s  |  %dfps  |  %dms", info.Text or "slimekrew", math.floor(smoothed + 0.5), ping)
    end)
    self.watermark = holder
    return holder
end

function library:SetWatermarkVisible(value)
    local holder = self:CreateWatermark()
...
function elementmethods:AddToggle(info)
    ...
    labelgradient:SetAttribute("ToggleGradient", true)
    ...
    local toggleglow = create("UIStroke", {Color = library.theme.GradientEnd, Thickness = 4, Transparency = 1, Parent = button})
    gradient(toggleglow)
    ...
    function object:SetValue(value, silent)
        ...
        tween(toggleglow, 0.25, {Transparency = self.Value and 0.58 or 1})
        if self.Value then
            label.TextTransparency = 1
            labelgradient.Enabled = gradientenabled
...
function elementmethods:AddColorPicker(info)
    info = info or {}
    local holder = rowholder(self, 24)
    local label = text(holder, info.Text or "Color", 10)
    ...
    local popup, popupscale, followconnection
    ...
    function object:SetValue(value, silent)
        ...
        self.Value = value
        tween(preview, 0.2, {BackgroundColor3 = value})
        if not silent then callback(self.Callback, value) end
    end
    local function closepopup()
        if followconnection then followconnection:Disconnect(); followconnection = nil end
        if popup then
            local oldpopup, oldscale = popup, popupscale
            popup, popupscale = nil, nil
            tween(oldpopup, 0.18, {GroupTransparency = 1})
            if oldscale then tween(oldscale, 0.2, {Scale = 0.9}) end
            task.delay(0.21, function() if oldpopup then oldpopup:Destroy() end end)
        end
    end
    connect(preview.MouseButton1Click, function()
        ...
        popup = create("CanvasGroup", {BackgroundColor3 = Color3.fromRGB(24, 25, 30), GroupTransparency = 1, Size = UDim2.fromOffset(220, 190), ZIndex = 110, Parent = root})
        popupscale = create("UIScale", {Scale = 0.9, Parent = popup})
        corner(popup, 5)
        local popupstroke = stroke(popup, library.theme.GradientEnd)
        gradient(popupstroke)
        tween(popup, 0.22, {GroupTransparency = 0})
        tween(popupscale, 0.28, {Scale = 1}, Enum.EasingStyle.Back)
        ...
        corner(sv, 3)
        local white = create("Frame", {BackgroundColor3 = Color3.new(1, 1, 1), Size = UDim2.fromScale(1, 1), ZIndex = 112, Parent = sv})
        corner(white, 3)
        ...
        local shade = create("Frame", {BackgroundColor3 = Color3.new(0, 0, 0), Size = UDim2.fromScale(1, 1), ZIndex = 113, Parent = sv})
        corner(shade, 3)
...
function library:CreateWindow(info)
    ...
    gradient(crumb)
    local subfluid = create("Frame", {BackgroundColor3 = library.theme.GradientStart, BackgroundTransparency = 0.16, BorderSizePixel = 0, Position = UDim2.fromOffset(160, 11), Size = UDim2.fromOffset(0, 26), Visible = false, ZIndex = 1, Parent = topbar})
    corner(subfluid, 99)
    gradient(subfluid)
    local subfluidstroke = stroke(subfluid, library.theme.GradientEnd, 0.08)
    ...
    local pageholder = create("Frame", {BackgroundColor3 = library.theme.Background, BorderSizePixel = 0, Position = UDim2.fromOffset(54, 48), Size = UDim2.new(1, -54, 1, -48), ClipsDescendants = true, Parent = main})
    ...
    local brand = text(sidebar, "slimekrew", 12, Color3.fromRGB(27, 28, 33))
    brand.Position = UDim2.fromOffset(50, 13)
    brand.Size = UDim2.fromOffset(92, 32)
    ...
    corner(statuscard, 7)
    stroke(statuscard)
    local statusscale = create("UIScale", {Scale = 0.9, Parent = statuscard})
    ...
    function window:SetVisible(value)
        ...
        if self.visible then
            ...
            tween(mainstroke, 0.18, {Transparency = 0})
        else
            tween(mainstroke, 0.08, {Transparency = 1})
            tween(mainscale, 0.28, {Scale = 0.88}, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
            tween(main, 0.25, {GroupTransparency = 1})
            task.delay(0.29, function() if not window.visible then main.Visible = false end end)
        end
    end
    ...
    function window:AddTab(name, icon)
        ...
        tablabel.ZIndex = 2
        fallback.TextTransparency = 0.58
        tooltip(button, name)
        ...
        function tab:AddSubtab(subname)
            ...
            local subbutton = create("TextButton", {AutomaticSize = Enum.AutomaticSize.X, BackgroundTransparency = 1, Font = Enum.Font.Code, Size = UDim2.fromOffset(0, 26), Text = subname, TextColor3 = Color3.new(1, 1, 1), TextSize = 9, Visible = false, ZIndex = 3, Parent = subbar})
            ...
            function subtab:AddSection(titlevalue, side)
                ...
                gradient(heading)
                local collapse = create("TextButton", {AnchorPoint = Vector2.new(1, 0.5), BackgroundTransparency = 1, Font = Enum.Font.Code, Position = UDim2.new(1, -7, 0, 14), Size = UDim2.fromOffset(20, 20), Text = ">", TextColor3 = library.theme.Text, TextSize = 12, Rotation = 90, Parent = holder})
                ...
                gradient(line)
                local sectioncontent = create("CanvasGroup", {AutomaticSize = Enum.AutomaticSize.Y, BackgroundTransparency = 1, GroupTransparency = 0, Position = UDim2.fromOffset(0, 29), Size = UDim2.new(1, 0, 0, 0), Parent = holder})
                ...
                connect(collapse.MouseButton1Click, function()
                    ...
                    tween(collapse, 0.25, {Rotation = section.open and 90 or 0}, Enum.EasingStyle.Back)
                    if section.open then
                        sectioncontent.Visible = true
                        sectioncontent.GroupTransparency = 1
                        tween(sectioncontent, 0.28, {GroupTransparency = 0})
                    else
                        tween(sectioncontent, 0.22, {GroupTransparency = 1})
                        task.delay(0.23, function() if not section.open then sectioncontent.Visible = false end end)
                    end
                end)
    ...
    function window:SelectSubtab(tab, subtab)
        for _, item in ipairs(tab.subtabs) do
            ...
            tween(item.button, 0.24, {TextTransparency = item == subtab and 0 or 0.4, TextColor3 = Color3.new(1, 1, 1)})
        end
        task.defer(function()
    ...
    connect(avatar.MouseButton1Click, function()
        ...
        tween(statusscale, 0.35, {Scale = 1}, Enum.EasingStyle.Back)
    end)
    connect(back.MouseButton1Click, function()
        tween(statusscale, 0.22, {Scale = 0.9})
        closeoverlays()
    end)
...
function library:CreateThemeStudio()
    ...
    global:AddColorPicker({Text = "Gradient Start", Default = Color3.fromRGB(255, 255, 255), Callback = function(value) library:SetTheme(value, library.theme.GradientEnd) end})
    global:AddColorPicker({Text = "Gradient End", Default = Color3.fromRGB(142, 147, 158), Callback = function(value) library:SetTheme(library.theme.GradientStart, value) end})
    ...
    menu:AddColorPicker({Text = "Menu Start", Default = Color3.fromRGB(30, 31, 36), Callback = function(value) library:SetMainMenuGradient({Start = value}) end})
    menu:AddColorPicker({Text = "Menu End", Default = Color3.fromRGB(48, 50, 57), Callback = function(value) library:SetMainMenuGradient({Finish = value}) end})
    ...
    for _, data in ipairs({{"Background", Color3.fromRGB(5, 5, 7)}, {"Surface", Color3.fromRGB(12, 12, 15)}, {"Surface2", Color3.fromRGB(18, 18, 23)}, {"Outline", Color3.fromRGB(42, 43, 51)}, {"Text", Color3.fromRGB(240, 241, 244)}}) do
        scheme:AddColorPicker({Text = data[1], Default = data[2], Callback = function(value) library:SetSchemeColor(data[1], value) end})
    end
...
function library:SaveProfile(name)
    ...
    for flag, option in pairs(self.options) do
        if option.Type == "Toggle" or option.Type == "Slider" or option.Type == "Dropdown" or option.Type == "MultiDropdown" or option.Type == "Input" or option.Type == "ColorPicker" then profile[flag] = option.Value end
    end
...
function library:SetTheme(startcolor, endcolor)
    ...
    for _, window in ipairs(windows) do
        ...
        tween(window.mainstroke, 0.45, {Color = self.theme.GradientEnd})
    end
end

function library:SetGradientSpeed(value)
...
function library:SetGradientsEnabled(value)
    ...
    for _, object in ipairs(gradients) do
        if object.Parent and not object:GetAttribute("ToggleGradient") then object.Enabled = gradientenabled end
    end
end
